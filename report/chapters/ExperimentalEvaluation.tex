...
opinion expressed by tweets might be both delayed or in respect to news. 
E.g. if a movie becomes popular it is likely that news spread very fast
on twitter, but slower on newspaper; whereas other topics are discussed on
twitter only after related news have been published for several days.

Thus, we considered a enlarged time window for news which starts 5 days before the
...
In table \ref{tab:resultsTfIdf} it is possible to see the results of TF/IDF. TF/IDF is interesting since it's widely used to compare other methods; it's results can be used as baseline comparison for other methods.
...
For the first three approaches time performance is not really relevant since it's all experiments were performed in few seconds or less, making them really suitable for real time processing. N-Gram Graph, on the other hand, is more resources consuming and so we made more detailed investigations about response time.
