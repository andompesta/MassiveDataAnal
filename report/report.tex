...
\maketitle
\begin{abstract}
In this report we consider the task to extract a summary of the main event that
caused a shift on the opinion of Twitter users. In particular we refer to those
shift as \emph{sentiment shift} and we try to capture the event that caused
them. In this work we present different language-independent and semantic-less
meteodologies and we try to figure out the pros and cons of each of them.
\end{abstract}  

\section*{Introduction}
This work aims in founding a summary of the events which caused a\emph{sentiment
shift}. 

While event extraction NLP technique are mature, their performance on tweets
inevitably degrades due to the inherent sparsity in short texts.  Since tweets
contain heterogeneous language structures and are at most 140 characters long,
to extract event from tweets is quite tricky: instead linking tweet to news allow us to extract event form news text that have a well formed structure and contains more text respect to tweet.
...
\begin{itemize}
	\item a simple \emph{bag of words} approach exploiting the \emph{SpaceSaving
		algorithm} \cite{SS}
